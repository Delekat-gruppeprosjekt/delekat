...
export default function RecipeCardProfile({ recipe, onEdit, onDelete, isAdmin, isOwnProfile }) {
  ...
  const handleDelete = async () => {
    if (!isAdmin && !isOwnProfile) {
      showToast("Du har ikke tillatelse til å slette denne oppskriften.", "error");
      return;
    }
  ...
  const confirmDelete = async () => {
    try {
      ...
      showToast("Oppskriften ble slettet.", "success");
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        message="Er du sikker på at du vil slette denne oppskriften?"
      />
    </>
  );
}
