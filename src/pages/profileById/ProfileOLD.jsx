...
function ProfilePage() {
  ...
  useEffect(() => {
    ...
    if (user) {
      // Fetch user details and recipes from Firestore
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      setAvatarUrl(newAvatarUrl || avatarUrl); // Update avatar URL in the state
  ...
  return (
    <div className="min-h-screen bg-BGcolor p-6">
      ...
      <div id="profileContainer" className="flex flex-col justify-center items-center my-16">
        ...
        <p>{auth.currentUser?.email || "Email"}</p>
        <p className="w-3/4 text-center">
          Bio: {newBio || "No bio set."}
        </p>
      </div>
